  ...
  useEffect(() => {
    if (person) {
      setFormData({
        name: person.name,
        email: person.email,
        type: person.type as 'EMPLOYEE' | 'CONTRACTOR',
        roleId: person.role.id,
        seniority: person.seniority as 'JUNIOR' | 'MID' | 'SENIOR' | 'PRINCIPAL' | 'PARTNER',
        ...
        practiceIds: person.practices.map(p => p.practiceId),
        notes: person.notes || '',
      });
    }
  }, [person]);
